...
namespace Bombermaaan 
{
    ...
    public class CControls : CModeScreen
    {
        // Duration (in seconds) of the two black screens
        private const float CONTROLS_BLACKSCREEN_DURATION         = 0.750f;

        // Display origin of the screen
        private const int CONTROLS_DISPLAY_ORIGIN_X               = 0;
        private const int CONTROLS_DISPLAY_ORIGIN_Y               = 0;

        // Sprite layers
        private const int CONTROLS_TEXT_SPRITE_LAYER              = 1;
        private const int CONTROLS_CURSOR_HAND_SPRITE_LAYER       = 1;

        private const int SPRITE_CURSOR_HAND                      = 0;

        private const float TIME_TO_WAIT_BEFORE_READING_INPUT     = 0.200f;

        private const int NUMBER_OF_MENU_ITEMS                    = 7;
        private const int FIRST_MENU_ITEM                         = 0;
        private const int LAST_MENU_ITEM                          = NUMBER_OF_MENU_ITEMS - 1;

        private const int DEVICE_MENU_ITEM                        = FIRST_MENU_ITEM;
        private const int UP_MENU_ITEM                            = 1;
        private const int DOWN_MENU_ITEM                          = 2;
        private const int LEFT_MENU_ITEM                          = 3;
        private const int RIGHT_MENU_ITEM                         = 4;
        private const int ACTION1_MENU_ITEM                       = 5;
        private const int ACTION2_MENU_ITEM                       = 6;

        private const string DEVICE_MENU_ITEM_STRING              = "DEVICE";
        private const string UP_MENU_ITEM_STRING                  = "UP";
        private const string DOWN_MENU_ITEM_STRING                = "DOWN";
        private const string LEFT_MENU_ITEM_STRING                = "LEFT";
        private const string RIGHT_MENU_ITEM_STRING               = "RIGHT";
        private const string ACTION1_MENU_ITEM_STRING             = "ACTION1";
        private const string ACTION2_MENU_ITEM_STRING             = "ACTION2";

        private const int FIRST_PLAYER_INPUT                      = 0;

        private const int SCREEN_TITLE_POSITION_Y                 = 20 + 80;
        private const int FIRST_MENU_ITEM_POSITION_Y              = 60 + 95;
        private const int ALL_MENU_ITEMS_POSITION_X               = 33 + 110;
        private const int SPACE_Y_BETWEEN_MENU_ITEMS              = 21;
        private const int SPACE_X_FROM_MENU_ITEM_TO_VALUE         = 90;
        private const int SPACE_X_FROM_MENU_ITEM_TO_CURSOR_HAND   = -25;
        private const int SPACE_Y_FROM_MENU_ITEM_TO_CURSOR_HAND   = -2;

        private const string SCREEN_CONTROLS_TITLE_STRING         = "CONTROLS";
        ...
        public override void Create()
        {
            ...
            m_pMosaic = CRandomMosaic.CreateRandomMosaic(m_pDisplay,
                                                          ...
                                                          MOSAIC_SPEED_X,
                                                          MOSAIC_SPEED_Y,
                                                          EMosaicColor.MOSAICCOLOR_PURPLE,
        ...
        public override EGameMode Update()
        {
            ...
            else if (!m_HaveToExit)
            {
                ...
                if (!m_SongStarted)
                {
                    ...
                    m_pSound.PlaySong(ESong.SONG_CONTROLS_MUSIC);
                ...
                if (m_WaitingForInputAfter <= 0.0f && !m_WaitingForInput)
                {
                    ...
                    if (m_pInput.GetMainInput().TestBreak())
                    {
                        ...
                        m_pSound.StopSong(ESong.SONG_CONTROLS_MUSIC);
                    ...
                    else if (m_pInput.GetMainInput().TestNext())
                    {
                        ...
                        if (m_Cursor != 0)
                        {
                            ...
                            m_WaitingForInputAfter = TIME_TO_WAIT_BEFORE_READING_INPUT;
                        }
                    }
                ...
                else
                {
                    ...
                    if (m_WaitingForInput)
                    {
                        ...
                        if (ActivatedControl != CInput.NO_ACTIVATED_CONTROL)
                        {
            ...
            return EGameMode.GAMEMODE_CONTROLS;
        }
        ...
        public override void Display()
        {
            ...
            else if (!m_HaveToExit)
            {
                ...
                m_Font.DrawCenteredX(0, CDisplay.VIEW_WIDTH, SCREEN_TITLE_POSITION_Y, SCREEN_CONTROLS_TITLE_STRING);

                // Position Y on the screen of the first menu item in this screen
                int MenuItemPositionY = FIRST_MENU_ITEM_POSITION_Y;

                string ItemString;   // Name of a menu item
                string ValueString;  // Name of the value corresponding to a menu item
                ...
                for (int MenuItemIndex = 0; MenuItemIndex < NUMBER_OF_MENU_ITEMS; MenuItemIndex++)
                {
                    ...
                    switch (MenuItemIndex)
                    {
                        ...
                        case UP_MENU_ITEM:
                        {
                            ...
                            ValueString = m_pInput.GetPlayerInput(m_PlayerInput).GetControlName(m_pOptions.GetControl(m_PlayerInput, EControl.CONTROL_UP));
                            break;
                        }
                        ...
                        case DOWN_MENU_ITEM:
                        {
                            ...
                            // Value : name of the control on the selected player input
                            ValueString = m_pInput.GetPlayerInput(m_PlayerInput).GetControlName(m_pOptions.GetControl(m_PlayerInput, EControl.CONTROL_DOWN));
                            break;
                        }
                        ...
                        case LEFT_MENU_ITEM:
                        {
                            ...
                            // Value : name of the control on the selected player input
                            ValueString = m_pInput.GetPlayerInput(m_PlayerInput).GetControlName(m_pOptions.GetControl(m_PlayerInput, EControl.CONTROL_LEFT));
                            break;
                        }
                        ...
                        case RIGHT_MENU_ITEM:
                        {
                            ...
                            // Value : name of the control on the selected player input
                            ValueString = m_pInput.GetPlayerInput(m_PlayerInput).GetControlName(m_pOptions.GetControl(m_PlayerInput, EControl.CONTROL_RIGHT));
                            break;
                        }
                        ...
                        case ACTION1_MENU_ITEM:
                        {
                            ...
                            // Value : name of the control on the selected player input
                            ValueString = m_pInput.GetPlayerInput(m_PlayerInput).GetControlName(m_pOptions.GetControl(m_PlayerInput, EControl.CONTROL_ACTION1));
                            break;
                        }
                        ...
                        case ACTION2_MENU_ITEM:
                        {
                            ...
                            // Value : name of the control on the selected player input
                            ValueString = m_pInput.GetPlayerInput(m_PlayerInput).GetControlName(m_pOptions.GetControl(m_PlayerInput, EControl.CONTROL_ACTION2));
                            break;
                        }
                    ...
                    m_Font.SetTextColor(EFontColor.FONTCOLOR_GREEN);

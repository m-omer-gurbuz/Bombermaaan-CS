...
namespace Bombermaaan
{
    ...
    public class CMenuInput : CMenuBase
    {
        private const int MENUINPUT_SPRITELAYER     = 1;       // Sprite layer where to draw sprites

        private const int TITLE_TEXT_POSITION_Y     = 90;      // Position Y of the title text that is centered on the X axis
        private const int INITIAL_TEXT_POSITION_X   = 191;     // Initial position of the player input's name
        private const int INITIAL_TEXT_POSITION_Y   = 77 + 90;

        private const int TEXT_SPACE_Y              = 21;      // Y Space in pixels between each text Y position

        private const int BOMBER_HEAD_SPACE_X       = -29;     // Space in pixels between the player input name text position
        ...
        private const int CURSOR_HAND_SPACE_X       = -54;     // Space in pixels between the player input name text position
        ...
        private const string TITLE_STRING           = "INPUT";  // String of the menu's title centered on the X axis

        // Player the cursor hand is currently pointing to
        private int m_CursorPlayer;

        public CMenuInput() : base()
        {
            m_CursorPlayer = 0;
        }
        ...
        protected override void OnCreate()
        {
            ...
            while (m_pOptions.GetBomberType(m_CursorPlayer) != EBomberType.BOMBERTYPE_MAN)
            {
